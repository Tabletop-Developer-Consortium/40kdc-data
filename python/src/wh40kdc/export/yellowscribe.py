...
def _unit_stat_profiles(view: Any) -> list[_El]:
    ...
    for i, p in enumerate(view.raw["profiles"]):
        ...
        out.append(
            _el("profile", [("name", name), ("typeName", "Unit")], [
                _el("characteristics", [], [
...
def _ability_profile(name: str, description: str) -> _El:
    return _el("profile", [("name", name), ("typeName", "Abilities")], [
        _el("characteristics", [], [
...
def _weapon_profiles(weapon: Any) -> list[_El]:
    ...
    for i, p in enumerate(weapon.raw["profiles"]):
        ...
        out.append(
            _el("profile", [("name", p["name"]), ("typeName", type_name)], [
                _el("characteristics", [], [
                    ...
                    _leaf(
                        "characteristic",
                        [("name", skill_name)],
                        _fmt_target(skill) if isinstance(skill, int) and not isinstance(skill, bool) else "N/A",
                    ),
...
def _unit_selection(
    ...
    children.append(_el("selections", [], _model_selections(unit, unit_id, view, dataset, faction_id)))
