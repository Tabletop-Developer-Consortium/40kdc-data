...
def _select_units_subject(sel: Any) -> str:
    ...
    within = f' within {_jstr(sel.get("within_inches"))}"' if sel.get("within_inches") is not None else ""
...
def _selection_eligibility(sel: dict[str, Any]) -> str:
    ...
    return f" {describe_selection_eligibility(eligibility)}" if isinstance(eligibility, dict) else ""
...
def _describe_effect_inline_base(e: Effect, ctx: Ctx | None = None) -> str:
    ...
    if etype == "recovery-pool":
        ...
        return (
            ...
            "the unit is at full strength and all its models have their full wounds; any unallocated "
            "points are lost"
        )
