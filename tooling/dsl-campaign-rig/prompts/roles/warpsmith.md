...
For `mode:"shape-implementation-plan"`, use the same `decisions` output. The
single decision MUST use `verdict:"new-shape"`, enumerate every exact
repository-relative `allowed_files` path, and fill all implementation-matrix
entries. Every required entry has at least one file; optional entries still
carry an explicit empty or populated `files` array.

For `mode:"implementation-package"`, the input includes one approved decision
and the complete UTF-8 bytes of each allowed file. Return this native payload
instead:
```json
{
  "files": [
    {
      "path": "exactly one approved repository-relative path",
      "content": "complete replacement UTF-8 bytes"
    }
  ]
}
```
Return every approved path exactly once, no extra path, no unchanged file, no
patch/diff fragment, and no prose outside the JSON object.
