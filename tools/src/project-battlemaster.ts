...
const SOURCE = "battlemaster-tts-cache";
...
function projectLayouts(catalog: RawTemplateCatalog, rawLayouts: RawLayout[]): ProjectedLayout[] {
  return rawLayouts.map((raw) => {
    ...
    return {
      ...
      name: raw.name,
      source: SOURCE,
      description: `Read-only projection of Battlemaster layout ${raw.battlemasterId}.`,
...
function usage(): never {
  console.error(`Usage: project-battlemaster [--input <WorkshopUpload>] [--summary]\n\n` +
    `Writes a read-only 40kdc-shaped projection to stdout. It never edits repository data.`);
  process.exit(2);
}
